...
<table class="table table-bordered table-condensed">
    ...
    <tr>
        ...
        <td class="left">
            <input type="text" name="ref_user" value="<?php echo $ref[0]; ?>">&nbsp;<input type="text" name="ref_memo"
                placeholder="No Memo" value="<?php echo $ref[1]; ?>">&nbsp;<input type="text" name="ref_tgl"
                id="ref_tgl" placeholder="Tanggal" value="<?php echo $ref[2]; ?>"><span
                style="color:red">*</span><i>Wajib di isi untuk perhitungan SLA</i><br>
            <input type="text" name="ref_hrbp" value="<?php echo $ref_hrbp[0]; ?>">&nbsp;<input type="text"
                name="ref_hrbp_memo" placeholder="No Memo" value="<?php echo $ref_hrbp[1]; ?>">&nbsp;<input type="text"
                name="ref_hrbp_tgl" id="ref_hrbp_tgl" placeholder="Tanggal" value="<?php echo $ref_hrbp[2]; ?>">
        </td>
    </tr>
    <tr>
        <td valign="top">Tanggal Pelaksanaan <span style="color:red">*</span></td>
        <td class="left">
            <input type="text" name="tgl_mulai" class="text input-small" id="start_date" placeholder="Tanggal Mulai"
                value="<?php echo $plan['tgl_mulai']; ?>" /> s/d
            <input type="text" name="tgl_selesai" class="text input-small" id="end_date" placeholder="Tanggal Selesai"
                value="<?php echo $plan['tgl_selesai']; ?>" />
        </td>
    </tr>
    ...
    <tr>
        ...
        <td class="left">
            <input type="text" name="kuota_kelas" class="text" value="<?php echo $plan['kuota_kelas']; ?>" />
        </td>
    </tr>
    <tr>
        ...
        <td class="left">
            ...
            <a href="location/list_city" class="btn btn-success" target="_blank" style="float:right">Tambah Lokasi Baru
                &nbsp;<i class="icon icon-pencil icon-white"></i></a>
        </td>
    </tr>
    <tr>
        ...
        select: function(event, ui) {
            if (ui.item.id != 0) {
                $("#result").append(
                    ...
                    "</a>&nbsp;<input type='text' name='jml_sesi[]' placeholder='Jumlah Sesi' class='input-small' />&nbsp;<input type='text' name='honor_sesi[]' placeholder='Honor / Sesi' class='input-medium' id='cur_pengajar'>&nbsp;<a href='#' class='remove_trainer'><i class='icon-remove'></i></a></td></tr>"
                );
        ...
        select: function(event, ui) {
            if (ui.item.id != 0) {
                $("#result_program").append(
                    "<tr><td><input type='hidden' value='" + ui.item.id +
                    "' name='kd_pelatihan'/>" + ui.item.value +
...
$(this).ready(function() {
    $("#kota").autocomplete({
        ...
        select: function(event, ui) {
            if (ui.item.id != 0) {
                $("#result_kota").append(
                    "<div><input type='hidden' value='" + ui.item.value +
                    "' name='lokasi_kota'/>" + ui.item.value +
                    "<a href='#' class='remove_kota'><i class='icon-remove'></i></a></div>"
                );
                $("#kota").hide();
                $("#null_kota").remove();
            }
...
<a href='provider/detail/" + ui.item.id + "' target='_blank'>" + ui.item.value +
                    "</a>&nbsp;\n\
