...
';                
    ...
    function _set_rules_tgihan() {
        ...
        $this->table->set_heading(
                'No', 'Program', 'Lembaga Provider', 'Tanggal Pelaksanaan', 'Tagihan Masuk', 'Tanggal Dokumen Lengkap', 'Kelengkapan','Tanggal Dokumen Ke GSFA','Nilai SLA','Pembayaran','Tanggal Pembayaran'
        );
 ...
 if (empty($row['tgl_tagihan_masuk']) or empty($row['tgl_dokumen_lengkap'])) {
     $kelengkapan=0;
        ...
        $tgl_pembayaran=$row['tgl_pembayaran'];    
            ...
            $this->table->add_row(
                ++$i,$row['program'], $row['provider'], $tgl_pelaksanaan, $this->editor->date_correct($row['tgl_tagihan_masuk']), $tgl_dokumen_lengkap, $kelengkapan, $tgl_dokumen_gsfa,$nilai_sla,$pembayaran,$tgl_pembayaran
            );
        }
