 ...
 */
class Peserta_induction extends Member_Controller {
    ...
    function index_pelatihan($offset=0) {
        ...
        /* Pagination */
        $config['base_url'] = site_url('peserta_induction/index_pelatihan/');
        ...
        foreach ($q as $row) {
            ...
            if ($this->mdl_peserta_induction->count_all($row['id'])<0) {
                ...
                $peserta=anchor('peserta_induction/view/'.$row['id'],$this->mdl_peserta_induction->count_all($row['id']));                
            }
        ...
        $this->template->display('peserta_induction/index_pelatihan', $data);
    }
	...
	if($id==null)
	{
	    ...
	    $data['action']='peserta_induction/add/'.$id;
            $data['action_manual']='peserta_induction/add_manual/'.$id;
	    $data['course_id']=$id;
	    $this->load->library('pagination');
	    if(empty($offset)){$offset=0;}
	    /* Pagination */
	    $config['base_url']=site_url('peserta_induction/view/'.$id);
	    $config['total_rows']=$this->mdl_peserta_induction->count_all($id);
	    $config['per_page']=$this->limit;
	    $config['uri_segment']=4;
	    $this->pagination->initialize($config);
	    $data['pagination']=$this->pagination->create_links();
            $data['kirim_email']='peserta_induction/kirim_email/'.$id;
            $data['pelatihan']=  $this->mdl_fgt_induction->get_by_id($id)->row_array();
	    ...
	    $peserta=$this->mdl_peserta_induction->get_index($id)->result_array();
	    $i=0+$offset;
            $data['edit_peserta_induction']='';
            $data['history_peserta_induction']='';
	    foreach ($peserta as $row)
	    {    
              $check_box='<input type="checkbox" name="check[]" value="'.$row['id'].'">';
            
              $data['edit_peserta_induction'].=$this->editor->edit_peserta_induction($row['id'],$offset);
              $data['history_peserta_induction'].=$this->editor->history_peserta_induction($row['nopek'],$data['pelatihan']['tgl_mulai']);
              $this->table->add_row(
			++$i,
                        $check_box,
			$row['nopek'].'<br>'.$this->cek_kategori($row['nopek']),
                        anchor('#edit_peserta_induction'.$row['id'],$row['nama_pekerja'], array('data-toggle'=>'modal')),
                        $row['position'],		
	    ...
	    $data['tiket']=$data['pelatihan']['kd_tiket'];
            $data['nama_pelatihan']= $this->mdl_pelatihan->get_by_id($data['pelatihan']['kd_pelatihan'])->row()->judul.'&nbsp; Batch &nbsp;'.$this->mdl_fgt_induction->get_by_id($id)->row()->batch;
            ...
            $data['download_absensi']= anchor('peserta_induction/absensi_peserta_induction/'.$id.'/'.$data['pelatihan']['kd_pelatihan'], 'Download Absensi', array('class'=>'btn btn-success'));           
            $this->template->display('peserta_induction/index',$data);
	}
    }
    
    
    function cek_peserta($nopek) {
    $peserta=$this->mdl_peserta_induction->get_by_nopek($nopek)->result_array();
   $pst=0;
    foreach ($peserta as $row) {
      $tgl_mulai=  $this->mdl_fgt_induction->get_by_id($row['id_fgt_induction'])->row()->tgl_mulai; 
    ...
    function cek_kategori($nopek) {
        if ($this->mdl_peserta_induction->cek_blacklist($nopek)>=2){
            $kategori=  anchor('peserta_induction/history/'.$nopek,'BlackList',array('class'=>'label label-important'));
        }elseif ($this->mdl_peserta_induction->cek_blacklist($nopek)==1) {
            $kategori=anchor('peserta_induction/history/'.$nopek,'GreyList',array('class'=>'label label-warning'));
        }  else {
            $kategori=anchor('peserta_induction/history/'.$nopek,'WhiteList',array('class'=>'label label-success'));
        }
        return $kategori;
    }
	...
	$this->load->library('pagination');
        if(empty($offset)){$offset=0;}
	/* Pagination */
	$config['base_url']=site_url('peserta_induction/history/');
	// $config['total_rows']=$this->mdl_fgt_induction->count_all_view($kd_pelatihan,$batch,$no_tiket,$tgl_awal,$tgl_selesai);
	$config['per_page']=$this->limit;
	$config['uri_segment']=3;
	$this->pagination->initialize($config);
	$data['pagination']=$this->pagination->create_links();
        $data['jml_pelatihan']=$this->mdl_peserta_induction->get_by_nopek($nopek)->num_rows();
	...
	$q=$this->mdl_peserta_induction->get_by_nopek($nopek)->result_array();
	$i=0+$offset;
	foreach ($q as $row)
	{
           $data['trans_pelatihan']= $this->mdl_fgt_induction->get_by_id($row['id_fgt_induction'])->row_array();            
        ...
        $this->table->set_template($tmpl);
        $data['content']=$this->table->generate();
        $data['peserta']=$this->mdl_peserta_induction->get_by_nopek($nopek)->row_array();
        $this->template->display('peserta_induction/history',$data);
    }
        ...
        $this->table->set_template($tmpl);
        $this->table->set_caption("ABSENSI PESERTA<br>".$this->mdl_pelatihan->get_by_id($kd_pelatihan)->row()->judul.'&nbsp; Batch '.$this->mdl_fgt_induction->get_by_id($id)->row()->batch.'<br>'.$this->mdl_fgt_induction->get_by_id($id)->row()->lokasi_kota.', '.$this->mdl_fgt_induction->get_by_id($id)->row()->tempat.'<br>'.$this->editor->date_correct($this->mdl_fgt_induction->get_by_id($id)->row()->tgl_mulai).' s/d '.$this->editor->date_correct($this->mdl_fgt_induction->get_by_id($id)->row()->tgl_selesai).'<br>');
        ...
        $q = $this->mdl_peserta_induction->get_absensi_peserta($id)->result_array();
        $i=0;
        foreach ($q as $row) {
    ...
    function kirim_email($id) {
     ...
     /*   if ($this->input->post('pilihan')=='mail') {
      $kd_pelatihan=  $this->mdl_fgt_induction->get_by_id($id)->row()->kd_pelatihan;
      $batch= $this->mdl_fgt_induction->get_by_id($id)->row()->batch;
     $to='';
     $cc='';
     ...
     $to.=$this->mdl_peserta_induction->get_peserta($peserta)->row()->email.';';    
     $cc.=$this->mdl_peserta_induction->get_peserta($peserta)->row()->email_atasan.';'.$this->mdl_peserta_induction->get_peserta($peserta)->row()->email_hr_area.';';    
     }
    ...
    $this->session->set_flashdata('msg',$this->editor->alert_error('Silahkan Pilih Salahsatu Data'));   
            ...
            foreach ($id_peserta as $key => $row) {
                $this->mdl_peserta_induction->update_konfirmasi($row,1);
                $this->update_tgl($row);
            }
        }elseif ($this->input->post('konfirmasi')=='konfirm_tdk_hadir') {
           
            foreach ($id_peserta as $key => $row) {
                $this->mdl_peserta_induction->update_konfirmasi($row,0);
                $this->update_tgl($row);
            }
        }elseif ($this->input->post('konfirmasi')=='absen_hadir') {
                       
            foreach ($id_peserta as $key => $row) {
                $this->mdl_peserta_induction->update_hadir($row,1);
                $this->update_tgl($row);
            }
        }elseif ($this->input->post('konfirmasi')=='absen_tdk_hadir') {
            foreach ($id_peserta as $key => $row) {
                $this->mdl_peserta_induction->update_hadir($row,0); 
                $this->update_tgl($row);
            }
        }
    ...
    function update_tgl($id) {
        $this->mdl_peserta_induction->update_tgl($id);
    }
    ...
    {        
        ...
        if (!empty($nopek)) {
            for ($a=0;$a<count($nopek);$a++){
                ...
                $this->mdl_peserta_induction->add($peserta);
            }
        }else {
        redirect('peserta_induction/view/'.$id);
        }	    
    ...
    function delete($id,$course_id) {
        $this->mdl_peserta_induction->delete($id);
        $this->session->set_flashdata('msg',$this->editor->alert_ok('Data berhasil dihapus'));
        redirect('peserta_induction/view/'.$course_id);
    }
    ...
    function edit_peserta($id,$id_pelatihan) {
        ...
        $this->mdl_peserta_induction->update($id,$peserta);
        redirect('peserta_induction/view/'.$id_pelatihan);
    }
        ...
        if (!$this->upload->do_upload()) {
          ...
          $data=  $this->spreadsheet_excel_reader->sheets[0];
            ...
            for ($i = 1; $i <= $data['numRows']; $i++) {
                ...
                $dataexcel[$i - 1]['id_fgt_induction'] = $id ;
            ...
            $this->mdl_peserta_induction->add_excel_pekerja($dataexcel);
            redirect("peserta_induction/view/".$id);
        }
