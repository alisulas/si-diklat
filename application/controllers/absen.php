...
class absen extends Member_Controller
{
    ...
    function detai($id_course)
    {
        ...
        $this->table->set_heading(
            ...
            array('data' => 'Nama'),
            array('data' => 'Nopek', 'width' => 200),
            array('data' => 'Direktorat', 'width' => 300),
            array('data' => 'Kehadiran', 'width' => 160),
            'Status'
        );
        ...
        foreach ($peserta as $rows) {
            $this->table->add_row(
                ...
                $rows['nopek'],
                $rows['direktorat'],
                $this->button_group($rows['id'], $rows['kehadiran']),
                array('data' => $this->ket($rows['kehadiran'], $rows['id']), 'id' => 'ket_' . $rows['id'])
            );
        }
